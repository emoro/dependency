#!/usr/bin/env Rscript
# Preprocess college closure simulation data for the web app.
# Outputs: public/colleges_poi.json (closed POIs), public/colleges_impact.json (POIs with impact)

library(readr)
library(jsonlite)

# Paths (run from project root)
poi_path    <- "data/colleges_POI.csv.gz"
impact_path <- "data/colleges_impact.csv.gz"
out_poi     <- "public/colleges_poi.json"
out_impact  <- "public/colleges_impact.json"
